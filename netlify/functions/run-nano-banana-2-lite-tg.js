// netlify/functions/run-nano-banana-2-lite-tg.js
// Nano Banana 2 Lite image launcher for Telegram Mini App.
...
const VERSION_TAG = "nano_banana_2_lite_tg_v1";
...
function normalizeImageSize(value) {
  ...
  const valid = new Set(["auto", "1:1", "1:4", "1:8", "2:3", "3:2", "3:4", "4:1", "4:3", "4:5", "5:4", "8:1", "9:16", "16:9", "21:9"]);
  ...
  return valid.has(coerced) ? coerced : "auto";
}
...
exports.handler = async function(event) {
  ...
  const modelLabel = "Nano Banana 2 Lite";
  ...
  const runId = String(body.run_id || body.runId || `nano2lite-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  try {
    ...
    await writeTelegramGeneration({ telegramId, cost, prompt, runId, taskId, modelLabel, kind: "image" });
