// netlify/functions/run-seedream-5-lite-tg.js
// Seedream 5.0 Lite launcher for Telegram Mini App.
// Backend owns pricing, creates the first telegram_generations row,
// then n8n updates that same row later by run_id.
...
const VERSION_TAG = "seedream_5_lite_tg_v1";
...
exports.handler = async function(event) {
  ...
  const imageUrls = rawUrls
    .map((u) => String(u || "").trim())
    .filter(Boolean)
    .map((u) => encodeURI(String(u)));

  const format = String(body.format || "png").toLowerCase();
  const size = normalizeImageSize(body.size || body.image_size || body.imageSize || body.aspect_ratio || body.aspectRatio);
  ...
  const runId = String(body.run_id || `sd5lite-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  try {
    ...
    if (!resp.ok) {
      return jsonResponse(resp.status || 502, {
        ok: false,
        submitted: false,
        error: (data && (data.error || data.message || data.msg)) || "seedream_5_lite_error",
        data,
        version: VERSION_TAG
      });
    }
    ...
    return jsonResponse(201, {
      ...
      new_credits: newCredits,
      cost,
      mode_used: hasImages ? "image_to_image" : "text_to_image",
      version: VERSION_TAG
    });
  } catch (e) {
