// netlify/functions/run-gpt-image-2-tg.js
// Submit a GPT Image 2 text/image-to-image job for Telegram Mini App.
...
exports.handler = async function(event){
  ...
  const hasImages = cleanUrls.length > 0;
  const model = hasImages ? "gpt-image-2-image-to-image" : "gpt-image-2-text-to-image";
  ...
  if (hasImages) {
    input.input_urls = cleanUrls.map(u => encodeURI(String(u)));
  }
  ...
  try {
    ...
    if (!resp.ok) {
      return jsonResponse(resp.status || 502, {
        ok:false,
        submitted:false,
        error:(data && (data.error || data.message || data.msg)) || "gpt_image_2_error",
        data
      });
    }
