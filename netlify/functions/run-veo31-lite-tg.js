// netlify/functions/run-veo31-lite-tg.js
// Veo 3.1 Lite video launcher for Telegram Mini App.
// Backend owns pricing. n8n updates telegram_generations later by run_id.

const KIE_BASE = (process.env.KIE_BASE_URL || "https://api.kie.ai").replace(/\/+$/, "");
const KIE_KEY = process.env.KIE_API_KEY || "";
const KIE_URL = `${KIE_BASE}/api/v1/veo/generate`;
...
const VERSION_TAG = "veo_31_lite_tg_v1";
...
function normalizeAspect(value) {
  return String(value || "16:9").trim() === "9:16" ? "9:16" : "16:9";
}

function costFor(body) {
  return normalizeResolution(body.resolution || body.quality) === "1080p" ? 2.5 : 2;
}
...
exports.handler = async function(event) {
  ...
  const modelLabel = "Veo 3.1 Lite Video";
  const resolution = normalizeResolution(body.resolution || body.quality);
  ...
  const runId = String(body.run_id || body.runId || `veo31lite-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  const payload = {
    prompt,
    model,
    aspect_ratio: normalizeAspect(body.aspectRatio || body.aspect_ratio),
    resolution,
  ...
  if (firstFrameUrl && lastFrameUrl) {
    ...
    payload.imageUrls = [firstFrameUrl, lastFrameUrl];
  } else if (firstFrameUrl || imageUrl) {
    const first = firstFrameUrl || imageUrl;
    payload.generationType = "FIRST_FRAME_2_VIDEO";
    payload.firstFrameUrl = first;
    payload.imageUrls = [first];
  } else {
    payload.generationType = "TEXT_2_VIDEO";
  }
  if (referenceImageUrls.length) payload.referenceImageUrls = referenceImageUrls;
  if (Number.isInteger(body.seeds)) payload.seeds = body.seeds;
  if (typeof body.enableFallback === "boolean") payload.enableFallback = body.enableFallback;
  if (typeof body.enableTranslation === "boolean") payload.enableTranslation = body.enableTranslation;
  if (typeof body.watermark === "string" && body.watermark) payload.watermark = body.watermark;
