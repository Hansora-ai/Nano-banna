// netlify/functions/run-seedance-2-mini-tg.js
// Seedance 2.0 Mini video launcher for Telegram Mini App.
...
const VERSION_TAG = "seedance_2_mini_tg_v1";
...
exports.handler = async function(event) {
  ...
  const modelLabel = "Seedance 2.0 Mini Video";
  const duration = clampDuration(body.duration);
  ...
  const runId = String(body.run_id || body.runId || `seedance2mini-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  const imageUrls = Array.isArray(body.reference_image_urls)
    ? body.reference_image_urls.map(String).filter(Boolean).slice(0, 9)
    : (Array.isArray(body.image_urls) ? body.image_urls.map(String).filter(Boolean).slice(0, 9) : []);
  const videoUrls = Array.isArray(body.reference_video_urls)
    ? body.reference_video_urls.map(String).filter(Boolean).slice(0, 3)
    : (Array.isArray(body.video_urls) ? body.video_urls.map(String).filter(Boolean).slice(0, 3) : []);
  const audioUrls = Array.isArray(body.reference_audio_urls)
    ? body.reference_audio_urls.map(String).filter(Boolean).slice(0, 3)
    : (Array.isArray(body.audio_urls) ? body.audio_urls.map(String).filter(Boolean).slice(0, 3) : []);
  ...
  const resolution = String(body.resolution || "720p").toLowerCase() === "480p" ? "480p" : "720p";
