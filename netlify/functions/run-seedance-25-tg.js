// netlify/functions/run-seedance-25-tg.js
// Seedance 2.5 video launcher for Telegram Mini App.
...
const VERSION_TAG = "seedance_25_tg_v1";
...
function normalizeSeedanceModel() {
...
function costFor(body) {
  ...
  return Number((duration * (resolution === "480p" ? 1.7 : 3.8)).toFixed(1));
}
...
exports.handler = async function(event) {
  ...
  const model = normalizeSeedanceModel();
  const modelLabel = "Seedance 2.5 Video";
  const duration = clampDuration(body.duration);
  ...
  const runId = String(body.run_id || body.runId || `seedance25-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  const imageUrls = Array.isArray(body.reference_image_urls)
    ? body.reference_image_urls.map(String).filter(Boolean).slice(0, 30)
    : (Array.isArray(body.image_urls) ? body.image_urls.map(String).filter(Boolean).slice(0, 30) : []);
  const videoUrls = Array.isArray(body.reference_video_urls)
    ? body.reference_video_urls.map(String).filter(Boolean).slice(0, 10)
    : (Array.isArray(body.video_urls) ? body.video_urls.map(String).filter(Boolean).slice(0, 10) : []);
  const audioUrls = Array.isArray(body.reference_audio_urls)
    ? body.reference_audio_urls.map(String).filter(Boolean).slice(0, 10)
    : (Array.isArray(body.audio_urls) ? body.audio_urls.map(String).filter(Boolean).slice(0, 10) : []);
  ...
  const input = {
    prompt,
    duration,
    aspect_ratio: hasFrameMode ? "adaptive" : String(body.aspect_ratio || body.aspectRatio || "1:1"),
    resolution,
  ...
  if (!hasFrameMode && imageUrls.length) input.reference_image_urls = imageUrls;
  if (!hasFrameMode && videoUrls.length) input.reference_video_urls = videoUrls;
  if (!hasFrameMode && audioUrls.length) input.reference_audio_urls = audioUrls;
