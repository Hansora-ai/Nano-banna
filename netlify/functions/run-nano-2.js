// netlify/functions/run-nano-2.js
// Submit a Nano Banana 2 text/image-to-image job for Telegram Mini App.
  ...
  try {
    ...
    if (!resp.ok) {
      return jsonResponse(resp.status || 502, {
        ok:false,
        submitted:false,
        error:(data && (data.error || data.message)) || "nano_banana_2_error",
        data
      });
    }
