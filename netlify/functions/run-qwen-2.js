// netlify/functions/run-qwen-2-tg.js
// Qwen 2 launcher for Telegram Mini App.
// Cost: 0.5 credit. n8n updates telegram_generations later by run_id.
...
const VERSION_TAG = "qwen_2_tg_v1";
...
exports.handler = async function(event) {
  ...
  const runId = String(body.run_id || body.runId || `qwen2-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  const input = {
    prompt,
    image_size: aspectToImageSize(aspectRatio),
    output_format: "png",
    seed: 0
  };

  if (isImageToImage) {
    input.image_url = encodeURI(inputUrls[0]);
  }
  ...
  try {
    ...
    if (!resp.ok || !taskId) {
      return jsonResponse(resp.status || 502, {
        ok: false,
        submitted: false,
        error: (data && (data.error || data.message || data.msg)) || "qwen_2_error",
        data,
        version: VERSION_TAG
      });
    }
