// netlify/functions/run-kling-3-turbo-tg.js
// Kling V3 Turbo video launcher for Telegram Mini App.
...
const VERSION_TAG = "kling_3_turbo_tg_v1";
...
function costFor(body) {
  ...
  return Number((duration * (resolution === "1080p" ? 1.5 : 1.2)).toFixed(1));
}
...
exports.handler = async function(event) {
  ...
  const runId = String(body.run_id || body.runId || `kling3turbo-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  const imageUrls = Array.isArray(body.image_urls)
    ? body.image_urls.map(String).filter(Boolean).slice(0, 1)
    : (Array.isArray(body.urls) ? body.urls.map(String).filter(Boolean).slice(0, 1) : []);
  const imageUrl = imageUrls[0] || String(body.image_url || body.imageUrl || body.fileUrl || "").trim();
  const hasImage = !!imageUrl;
  const resolution = String(body.resolution || "720p") === "1080p" ? "1080p" : "720p";
  const model = hasImage ? "kling/v3-turbo-image-to-video" : "kling/v3-turbo-text-to-video";
  const modelLabel = "Kling 3 Turbo Video";
