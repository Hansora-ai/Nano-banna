// netlify/functions/run-happyhorse-1-tg.js
// HappyHorse 1.0 video launcher for Telegram Mini App.
...
const VERSION_TAG = "happyhorse_1_tg_v1";
...
async function writeTelegramGeneration({ telegramId, cost, prompt, runId, taskId, modelLabel }) {
  ...
  try {
    const resp = await fetch(TG_TABLE_URL, {
      ...
      body: JSON.stringify([{
        telegram_id: telegramId,
        model: modelLabel || "HappyHorse 1.0 Video",
        credits: cost,
        prompt,
        run_id: runId,
        task_id: taskId || null,
...
exports.handler = async function(event) {
  ...
  const runId = String(body.run_id || body.runId || `happyhorse1-${telegramId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  const model = videoUrl
    ...
    : imageUrls.length || firstFrameUrl
  ...
  try {
    ...
    await writeTelegramGeneration({ telegramId, cost, prompt, runId, taskId, modelLabel: "HappyHorse 1.0 Video" });
